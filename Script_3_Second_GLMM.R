...
########################################################
# Example 3 data set - live native shrub density
########################################################
...
# Families to try: NB - two versions
# Trend over time
...
# which distribution has the better fit?
...
##############################################################################
NB2_fit_b <- glmmTMB(Y ~ <add terms here>, 
                        data = Vegetation,
                        REML = T, 
                        family=nbinom2(link='log')) 
summary(NB2_fit_b)
...
NB2_fit_c <- glmmTMB(Y ~ <add terms here>, 
                        data = Vegetation,
                        REML = T, 
                        family=nbinom2(link='log')) 
...
NB2_fit_d <- glmmTMB(Y ~ <add terms here>,
                        data = Vegetation,
                        REML = T, 
                        family=nbinom2(link='log')) 
...
# Which fit is preferred using AIC?
...
# We have assumed an unstructured random effects structure to account correlation
#   in which each pairwise correlation takes unique values
par(mfrow=c(3,3))
for(i in 1:200) {
